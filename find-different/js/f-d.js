;~function(){
    ...
    var d = document,
        ...
        tipsWordArr = ["美女来找茬！总共有4关，请找出上下图中4处不同之处，找不着可以放大看噢，点击开始游戏开始，点击暂停游戏可以休息噢" , "看累了，休息下吧！" , "时间到，游戏结束！每点错一次扣10s，挑战失败~" , "挑战成功！恭喜你解锁线索四：俄罗斯地图下的风景" , "开始游戏" , "暂停游戏"],
        imgNumArr = [],
        imgDiffNum = 0,
        imgFindNum = 0, 
    ...
    var game = {
        ...
        over : function(imgFindCount){
            if(imgFindCount === imgData.count - 1){ // you win
                sceneMask.show(tipsWordArr[3] );
            }
            else{ // time is over
                sceneMask.show(tipsWordArr[2] );
            }   
            counter.stop(); 
            isPlayAgain = true;
    ...
    var counter = {
        ...
        start : function(obj , isPlayAgain , callback){
            ...
            self.timer = setInterval(function(){
                ...
                obj.style["width"] = (self.num/90) * self.maxW + "px";
            },1000);
        },
        stop : function(){
